...
$(function(){
    window.addEventListener("message", function(event){   
        if(event.data.type == "show_game_ui") {
            ...
            // Speak("Pay the price of the contract!");
            $('#header').hide();
...
function showWinTeam(data, finished)
{
    ...
    if(data.win_team == "RedTeam") {
        $('#winning_team').html(`<span class="red">Evil Team</span>`);
        _textSpeak = "Η ομάδα του Κακού κέρδισε!";
    } else {
        $('#winning_team').html(`<span class="blue">Good Team</span>`);
        // _textSpeak = "Η καλή ομάδα κέρδισε!";
    }
    $('#who_won').fadeIn();
    if(finished) {
        $('.div_info').html(`The match will end in <span class="red" id="remain_time">15</span> seconds more!`);
        // _textSpeak += " Ο αγώνας θα λήξει σε 15 δευτερόλεπτα!";
    }
    // Speak(_textSpeak);
...
function startMatch() {
    ...
    //Speak("ο όπσο είναι λούστης!");
	new Audio("sounds/Welcome.mp3").play()
}

function Speak(text) {
    $('#speak_ai').attr("src", "https://translate.google.com/translate_tts?ie=UTF-8&q="+encodeURI(text)+"&tl=el-us&total=1&idx=0&client=tw-ob");
    // $('#speak_ai').play();
